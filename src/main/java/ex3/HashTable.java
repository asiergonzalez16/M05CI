...
public class HashTable {
    ...
    private HashEntry encontrarKeyExistente(String key) { //Método para encontrar una key repetida
    ...
    private void insertaNodo(HashEntry hashEntry, int hash) { //Añadirlo si no existe
        HashEntry temp = entries[hash];
    ...
    private void eliminarNodo(HashEntry entry, int hash) {
        if (entry.prev == null) {
    ...
    public void put(String key, String value) {
        ...
        if (entries[hash] == null) {
            ...
            HashEntry temp = encontrarKeyExistente(key);
            if (temp != null) {
                temp.value = value;
            } else {
                insertaNodo(hashEntry, hash);
            }
        }
    }
    ...
    public void drop(String key) {
        int hash = getHash(key);
        if(entries[hash] != null) {
            HashEntry entryABorrar = encontrarKeyExistente(key);
            if(entryABorrar != null) {
                eliminarNodo(entryABorrar, hash);
                ITEMS--;
            }
        }
    }
